...
6. **資料庫查詢**：如果用戶問題中包含資料庫查詢，請使用 custom_nl2sql_query 工具進行查詢，將 table_info_list 參數傳入，並將結果回傳
7. **後備回答**：如果用戶問題中包含資料庫查詢，但 custom_nl2sql_query 工具查詢結果為空或不理想，請使用後備回答，後備回答的內容需要包含建議查閱的資料集名稱以及網址"""
